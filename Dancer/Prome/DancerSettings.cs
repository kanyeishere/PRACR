...
public sealed class DancerSettings
{
    ...
    public void Save()
    {
        try
        {
            var directory = ACRAuthorSetting.GetSettingsDirectory("Wotou");
        ...
        {
            PluginLog.Error($"[Wotou] 保存舞者设置失败: {ex}");
        }
    }
    ...
    private static DancerSettings Load()
    {
        try
        {
            var path = Path.Combine(ACRAuthorSetting.GetSettingsDirectory("Wotou"), "DancerSettings.json");
            if (File.Exists(path))
                return JsonSerializer.Deserialize<DancerSettings>(File.ReadAllText(path), Options) ?? new();
        }
        catch (Exception ex)
        {
            PluginLog.Error($"[Wotou] 读取舞者设置失败，将使用默认值: {ex}");
        }
